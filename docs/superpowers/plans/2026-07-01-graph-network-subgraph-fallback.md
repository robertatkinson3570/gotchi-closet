...
- **NEXT: Task 6 (USER clicks Publish → Arbitrum One + creates domain-restricted, spend-capped API key `gotchicloset-fallback`)**, then Task 7 config half (gh secret set SUBGRAPH_URL_BACKUP, Vercel VITE_GOTCHI_SUBGRAPH_URL_BACKUP, deploy), then Task 8. Parity gate is CLEARED — user is clear to publish.
