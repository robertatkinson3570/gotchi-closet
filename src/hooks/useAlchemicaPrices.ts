...
let cache: { ts: number; data: AlchemicaPrices } | null = null;
let inflight: Promise<AlchemicaPrices> | null = null;
...
export function useAlchemicaPrices() {
  const [prices, setPrices] = useState<AlchemicaPrices>(
    cache?.data ?? ALCHEMICA_PRICES_GHST_FALLBACK
  );
  ...
  useEffect(() => {
    ...
    fetchAlchemicaPrices()
      .then((p) => {
        if (cancelled) return;
        setPrices(p);
        setIsLive(p !== ALCHEMICA_PRICES_GHST_FALLBACK);
        setLoading(false);
      })
      .catch(() => {
        if (cancelled) return;
