...
test("home routes to editor with manual + connected wallets", async ({ page }) => {
      ...
      owner === normalize(CONNECTED_ADDRESS)
        ...
        ? [
            ...
            },
            {
              ...
              name: "ManualGotchi2",
              level: "1",
              numericTraits: [40, 40, 40, 40, 50, 50],
              modifiedNumericTraits: [40, 40, 40, 40, 50, 50],
              equippedWearables: [0, 0, 0, 0, 0, 0, 0, 0],
              baseRarityScore: "346",
              hauntId: "1",
              collateral: "0x0000000000000000000000000000000000000000",
  ...
  await page.getByRole("button", { name: "Injected Wallet (MetaMask/Rabby)" }).click();

  await page.getByTestId("home-manual-input").fill(MANUAL_ADDRESS);
  await page.getByTestId("home-dress-btn").click();

  await expect(page).toHaveURL(new RegExp(`/dress\\?view=${MANUAL_ADDRESS}`));
