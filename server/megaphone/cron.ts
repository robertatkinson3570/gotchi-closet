...
import { markTweetFailed, markTweetPosted, pendingTweetPosts } from "./tweets";
...
async function reconcile(): Promise<void> {
  ...
  for (const t of pendingTweets) {
    ...
    const state = String(post.state ?? post.status ?? "").toUpperCase();
    const url = releaseUrl(post);
    if (state === "PUBLISHED" && url) markTweetPosted(t.id, url);
    else if (state === "ERROR") markTweetFailed(t.id);
  }
}
