...
- **Wearable Set Data Fixes:** The `data/wearableSets.json` file includes corrections for known-incorrect set data. Many sets had their trait bonuses incorrectly shifted (BRS value in NRG slot, modifiers scrambled). Fixed 27+ sets including Quickswap Dragon (BRS +3, AGG +1, SPK +1), Infantry, Link Marine, Godlike Sergey, Vitalik Visionary, and others per the official wiki at https://wiki.aavegotchi.com/en/sets.
